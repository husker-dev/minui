...
extern "C" {
    ...
    JNIEXPORT jobjectArray JNICALL Java_com_husker_minui_natives_impl_win_Win_nGetRegistryValues(JNIEnv *env, jobject, jlong hkey, jstring jpath) {
        HKEY hKey;
        auto status = RegOpenKeyEx((HKEY) hkey, (LPCSTR) env->GetStringUTFChars(jpath, nullptr), 0, KEY_READ, &hKey);
